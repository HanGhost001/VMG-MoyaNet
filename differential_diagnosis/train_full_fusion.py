"""
Differential Diagnosis Training Script (YAML-driven)

Train a DenseNet121 (or ResNet) model for full-brain differential diagnosis
(MMD vs ICAS vs NC).
...
def main():
    parser = argparse.ArgumentParser(description="Train differential diagnosis model")
    ...
    allow_flip_lr = cfg['data'].get('allow_flip_lr', True)
    ...
    val_freq = cfg.get('evaluation', {}).get('validation_frequency', 1)
    ...
    for epoch in range(1, epochs + 1):
        ...
        should_eval = (epoch % val_freq == 0) or (epoch == 1)
        if should_eval:
            val_m = evaluate(model, val_loader, device, num_classes)
            last_val_m = val_m
        else:
            val_m = last_val_m or {"loss": 0.0, "acc": 0.0, "bal_acc": 0.0, "macro_f1": 0.0}
        ...
        if use_warmup and epoch <= warmup_epochs:
            ...
            if scheduler is not None:
                if sched_type == 'cosine':
                    scheduler.step()
                elif sched_type == 'plateau' and should_eval:
                    scheduler.step(val_m['bal_acc'])
            current_lr = optimizer.param_groups[0]['lr']

        # Log
        epoch_s = time.perf_counter() - epoch_t0
        record = {"epoch": epoch, "train_loss": train_loss, "val": val_m if should_eval else None, "lr": current_lr}
        with open(history_path, "a", encoding="utf-8") as f:
            f.write(json.dumps(record, ensure_ascii=False) + "\n")

        suffix = "" if should_eval else " (last)"
        print(
            f"Epoch {epoch:03d} | train_loss={train_loss:.4f} | "
            f"val_loss={val_m['loss']:.4f} val_acc={val_m['acc']:.3f} val_bal_acc={val_m['bal_acc']:.3f} "
            f"val_f1={val_m['macro_f1']:.3f}{suffix} | lr={current_lr:.2e} | {epoch_s:.1f}s",
            flush=True,
        )

        # Best model / early stopping (only on eval epochs)
        if should_eval:
            should_update = False
